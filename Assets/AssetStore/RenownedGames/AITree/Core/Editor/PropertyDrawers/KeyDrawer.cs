...
namespace RenownedGames.AITreeEditor
{
    ...
    public sealed class KeyDrawer : FieldDrawer
    {
        ...
        private void SelectDropdown(Rect position, SerializedField serializedField)
        {
            ...
            window.SetSortType(ExSearchWindow.SortType.Alphabet);
        ...
        private void ValidateLocalField(SerializedField serializedField, ref SerializedField localKeyField)
        {
            ...
            catch
            {
                ...
                localKeyField.SetLabel(new GUIContent(GUIContent.none));
                if (localKeyField.GetSerializedProperty().hasVisibleChildren)
                {
                    localKeyField.AddView(new InlineEditorAttribute());
                }
            }
        }
                    ...
                    if (scriptableObject is Node node)
                    {
                        ...
                        if(currentTree != null)
                        {
                            foreach (TrackerWindow tracker in TrackerWindow.ActiveTrackers)
                            {
                                ...
                                {
                                    if (window.TryGetBehaviourTree(out BehaviourTree behaviourTree) && behaviourTree == currentTree)
                                    {
