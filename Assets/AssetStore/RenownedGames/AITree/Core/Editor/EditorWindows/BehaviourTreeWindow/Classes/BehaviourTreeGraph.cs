...
namespace RenownedGames.AITreeEditor
{
    ...
    [UxmlElement]
    public partial class BehaviourTreeGraph : GraphView
#else
...
#endif
    {
        ...
        private class ArrangeBox
        {
            ...
            private List<ArrangeBox> boxes;

            public ArrangeBox(BehaviourTreeGraph graph, NodeView target)
            {
                this.graph = graph;
                this.target = target;

                boxes = new List<ArrangeBox>();
                List<NodeView> views = GetChilderenViews();
                foreach (NodeView view in views)
                {
                    if (view.GetNode().Arrangeable())
                    {
                        ArrangeBox box = new ArrangeBox(graph, view);
                        boxes.Add(box);
                    }
                }
            }
            ...
            public void RecalculateSize(Vector2 margin)
            {
                if (target != null)
                {
                    boxes.ForEach(c => c.RecalculateSize(margin));
                    selfSize = target.GetPosition().size + margin;
                    size = Vector2.zero;

                    foreach (ArrangeBox box in boxes)
                    {
                        size.x += box.size.x;
            ...
            public void RecalculatePosition(Vector2 origin)
            {
                if (target != null)
                {
                    ...
                    position.y = origin.y;
                    target.SetPosition(new Rect(position, selfSize));

                    float childWidth = 0;
                    foreach (ArrangeBox box in boxes)
                    {
                        childWidth += box.size.x;
                    }
            ...
            private List<NodeView> GetChilderenViews()
            {
                ...
                if (target != null)
                {
                    if (target.GetNode() is CompositeNode composite)
                    {
                    ...
                    else if (target.GetNode() is RootNode root && root.GetChild() != null)
                    {
        ...
        public BehaviourTreeGraph() : base()
        {
            ...
            foreach (Manipulator manipulator in DefaultManipulators)
            {
            ...
            {
                miniMap = new MiniMapView() { windowed = true, graphView = this };
                Add(miniMap);
            }
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            else if (evt.target is BehaviourTreeGraph)
            {
                ...
                evt.menu.AppendAction("Paste", a => PasteCallback(), copyData != null ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
                evt.menu.AppendAction("Auto Arrange", a => AutoArrange(), AutoArrangeStatus);

                evt.menu.AppendSeparator();
                evt.menu.AppendAction("Select Asset", a => EditorGUIUtility.PingObject(behaviourTree), AssetDatabase.IsNativeAsset(behaviourTree) ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
            }
            else if (evt.target is NodeView nodeView)
            {
                ...
                if (wrapNode != null && selection.Count == 1)
                {
                    ...
                    if(wrapView != null)
                    {
                        ...
                        evt.menu.AppendSeparator();
                        evt.menu.AppendAction("Capabilities/Arrangeable", a =>
                        {
                            wrapNode.Arrangeable(!wrapNode.Arrangeable());
                        }, s =>
                        {
                            return wrapNode.Arrangeable() ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal;
                        });
                        ...
                        if (wrapView.GetNode() is TaskNode taskNode)
                        {
                            ...
                            if (taskNode is SubTree subTree)
                            {
                                evt.menu.AppendAction("Open In New Window", a =>
                                {
                                    BehaviourTreeWindow window = TrackerWindow.CreateTracker<BehaviourTreeWindow>();
                                    if (EditorApplication.isPlaying)
                                    {
                                        window.SubTreeIn(subTree.GetSubTree());
                                    }
                                    else if (subTree.GetSharedSubTree() != null)
                                    {
                                        window.SubTreeIn(subTree.GetSharedSubTree());
                                    }
                                },
                                subTree.GetSharedSubTree() != null ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);

                            }

                        }

                        bool canReplace = !EditorApplication.isPlaying || AssetDatabase.IsNativeAsset(behaviourTree);
                        evt.menu.AppendAction("Replace", a => ReplaceNode(nodeView, localMousePosition), canReplace ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
                    }
                }

                evt.menu.AppendSeparator();
                if(nodeView.GetNode() is WrapNode)
                {
                ...
                if (selection.Count == 1)
                {
                    ...
                    evt.menu.AppendAction("Select Script", a => SelectNodeScript(nodeView.GetNode()));
                }
            }
        }
        ...
        public void AutoArrange()
        {
            ...
            if(view != null)
            {
                view.schedule.Execute(() => AutoArrange(view, Vector2.zero, Vector2.one * 39));
            }
        }
        ...
        /// <summary>
        /// Select the node script in project window.
        /// </summary>
        /// <param name="node">Node reference.</param>
        internal void SelectNodeScript(Node node)
        {
            MonoScript monoScript = MonoScript.FromScriptableObject(node);
            if (monoScript != null)
            {
        ...
        private void HotKeyListener()
        {
            if (HotKeyUtility.TryGetEvent(window, out Event evt))
            {
                if(evt.keyCode == KeyCode.Delete
                    || (evt.keyCode == KeyCode.Backspace && (evt.control || evt.command)))
                {
        ...
        private void ReplaceNode(NodeView nodeView, Vector2 localMousePos)
        {
            Node node = nodeView.GetNode();
            Vector2 windowPosition = GUIUtility.GUIToScreenPoint(localMousePos);
            OpenNodeCreationWindow(windowPosition, localMousePos, nv =>
            {
                int undoGroupIndex = Undo.GetCurrentGroup();
                Undo.IncrementCurrentGroup();

                Rect prevRect = nodeView.GetPosition();
                string title = nodeView.GetTitleLabel().text;
                Node parent = node.GetParent();
                List<Node> children = new List<Node>();
                if (node is CompositeNode compositeNode)
                {
                    children.AddRange(compositeNode.GetChildren());
                }

                List<DecoratorNode> decorators = new List<DecoratorNode>();
                List<ServiceNode> services = new List<ServiceNode>();
                if (node is WrapNode wrapNode)
                {
                    List<DecoratorNode> _decorators = wrapNode.GetDecorators();
                    List<ServiceNode> _services = wrapNode.GetServices();

                    if (_decorators.Count > 0 || _services.Count > 0)
                    {
                        Undo.RecordObject(behaviourTree, $"Replace decorators and services in behaviourTree.");
                        foreach (DecoratorNode decorator in _decorators)
                        {
                            DecoratorNode decoratorNode = decorator.Clone() as DecoratorNode;
                            if (!EditorApplication.isPlaying)
                            {
                                AssetDatabase.AddObjectToAsset(decoratorNode, behaviourTree);
                            }
                            decorators.Add(decoratorNode);
                        }

                        foreach (ServiceNode service in _services)
                        {
                            ServiceNode serviceNode = service.Clone() as ServiceNode;
                            if (!EditorApplication.isPlaying)
                            {
                                AssetDatabase.AddObjectToAsset(serviceNode, behaviourTree);
                            }
                ...
                Node _node = nv.GetNode();
                Undo.RecordObject(_node, $"Replace operations for new node.");
                _node.SetName(title);
                nv.SetPosition(prevRect);

                if (children.Count > 0 && _node is CompositeNode _compositeNode)
                {
                    _compositeNode.SetChildren(children);
                }

                if (_node is WrapNode _wrapNode)
                {
                    _wrapNode.SetDecorators(decorators);
                    _wrapNode.SetServices(services);
                }

                if (parent is CompositeNode composite)
                {
                    composite.AddChild(nv.GetNode());
                }
        ...
        /// <summary>
        /// Iteration of actually added manipulators.
        /// </summary>
        public IEnumerable<Manipulator> Manipulators
        {
            get
            {
                return manipulators;
        ...
        /// <br>Always return new instances of manipulators.</br>
        /// <br>See: <see cref="Manipulators"/></br>
        /// </summary>
        protected virtual IEnumerable<Manipulator> DefaultManipulators
        {
            get
            {
                yield return new ContentZoomer() { minScale = 0.25f, maxScale = 5.0f, scaleStep =settings.GetZoomStep()};
        ...
        private void OnDoubleClickSelection(VisualElement target)
        {
            ...
            if (nodeView != null)
            {
                ...
                {
                    window.SubTreeIn(EditorApplication.isPlaying ? subTree.GetSubTree() : subTree.GetSharedSubTree());
                }
        ...
        {
            if (copyData == null || copyData.nodesToCopy == null || behaviourTree == null)
            {
                return;
            }
        ...
        private DropdownMenuAction.Status AutoArrangeStatus(DropdownMenuAction action)
        {
            Node node = behaviourTree.GetRootNode();
        ...
        public BehaviourTreeWindow GetWindow()
        {
            return window;
        }

        internal void SetWindow(BehaviourTreeWindow value)
        {
            window = value;
        }
        #endregion
